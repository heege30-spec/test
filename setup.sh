...
#
# LƯU Ý QUAN TRỌNG (đã xác minh lại, sửa từ bản trước):
...
#
# Cách đúng: dùng gói build-sẵn riêng của Termux cho 3 thứ này — cài trong
# vài giây, không biên dịch gì cả:
#   pkg install python-numpy python-pillow python-opencv-python
# (KHÔNG dùng "opencv" — đó là gói C++ đầy đủ + contrib, nặng hơn nhiều lần
# so với "python-opencv-python" chỉ chứa binding cv2 cần dùng.)
# ============================================================
...
# của Termux (KHÔNG qua pip) — xem ghi chú đầu file: pip sẽ luôn cố biên
# dịch từ mã nguồn trên Termux và thất bại, đây là giới hạn nền tảng.
PKGS=(
  python
  clang
  cmake
