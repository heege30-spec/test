...
# LƯU Ý: KHÔNG dùng "pip install --upgrade pip" trên Termux — Termux tự quản lý
# gói python-pip qua pkg và CHỦ ĐỘNG chặn việc pip tự nâng cấp chính nó
# ("ERROR: Installing pip is forbidden, this will break the python-pip package").
# Muốn cập nhật pip thì dùng: pkg install -y python-pip
