...
# (Bổ sung: Stealth mode, human-like jitter/typing, emulator handling,
#  ổn định hơn cho adb/AI calls, nạp API key từ config hoặc nhập tạm)
# ============================================================
...
GEMINI_API_KEYS = [
    "jisonAQ.Ab8RN6JegJQYhzXtF6RywbHytiS6B2CPRChwtZ1MSnYc-Fg1lA",  # key you provided
...
STEALTH_MODE = False                   # mặc định tắt; có thể bật qua config.json
STEALTH_JITTER_PIXELS = 6              # jitter khi tap +/- pixels
STEALTH_MIN_DELAY = 0.03
...
def _has_any_valid_key(*key_lists) -> bool:
...
def _prompt_api_keys_interactively():
    global GEMINI_API_KEYS, QWEN_API_KEYS
    if not sys.stdin.isatty():
        return

    log("🔒 Chưa tìm thấy API key hợp lệ nào (không có trong config.json, không sửa cứng trong code).")
    log("   Key gõ ở đây sẽ KHÔNG hiện lên màn hình và CHỈ nằm trong RAM phiên chạy này — không ghi ra file nào.")

    try:
        gemini_key = getpass.getpass("Nhập Gemini API key (Enter để bỏ qua): ").strip()
    except (EOFError, KeyboardInterrupt):
        gemini_key = ""
    try:
        qwen_key = getpass.getpass("Nhập Qwen API key (Enter để bỏ qua): ").strip()
    except (EOFError, KeyboardInterrupt):
        qwen_key = ""
    ...
    if qwen_key:
        QWEN_API_KEYS = [qwen_key]


if not _has_any_valid_key(GEMINI_API_KEYS, QWEN_API_KEYS):
    _prompt_api_keys_interactively()
...
# ============================================================
# LỚP TIỆN ÍCH: điều khiển ADB (với Stealth/Jitter + ổn định)
# ============================================================
...
def detect_emulator(force_reload: bool = False) -> bool:
    ...
    with _EMULATOR_CHECK_LOCK:
        ...
        try:
            ...
            lower = props.lower()
            indicators = [
                "ro.kernel.qemu", "generic sdk", "emulator", "virtual", "ro.product.model",
                "genymotion", "goldfish", "ranchu", "sdk_gphone", "sdk"
            ]
...
# ============================================================
# Lớp 1a/1b: Accessibility + Image match bằng Pillow + numpy
# ============================================================
...
# ============================================================
# LỚP 2: GỌI API AI (Gemini via REST + Qwen)
# ============================================================
def _call_gemini_with_key(key: str, prompt: str) -> str:
    ...
    try:
        ...
        raise RuntimeError(f"Phản hồi Gemini không đúng định dạng mong đợi: {data}") from e
