...
# (ĐÃ BỔ SUNG: Stealth mode, human-like jitter/typing, emulator handling)
# ============================================================
# Kiến trúc: (giữ nguyên mô tả trong file gốc)
# ...
import time
import json
import os
...
# ============================================================
# Bật Stealth mode để mô phỏng hành vi giống người (jitter, typing delays, long-press)
STEALTH_MODE = False

# Jitter (pixels): thêm nhiễu +/- khi tap
STEALTH_JITTER_PIXELS = 6

# Delay trước/sau hành động (seconds)
STEALTH_MIN_DELAY = 0.05
STEALTH_MAX_DELAY = 0.35

# Khi typing, tỉ lệ dùng chế độ typing "nhân tạo" (ký tự/ký tự)
STEALTH_HUMAN_TYPING = True
STEALTH_TYPING_MIN = 0.03
STEALTH_TYPING_MAX = 0.18

# Tỉ lệ ngẫu nhiên để chuyển thành long-press thay vì tap (mimic)
STEALTH_LONG_PRESS_PROB = 0.04
STEALTH_LONG_PRESS_DURATION_MS = 700  # dùng input swipe x y x y duration để giả long-press

# Khi phát hiện là emulator, cho phép thêm các hành vi "giả lập" (rung, lắc, long-press)
EMULATOR_BEHAVIOR_ENABLED = True
EMULATOR_LONG_PRESS_DURATION_MS = 1000
EMULATOR_SHAKE_ENABLED = True

# ============================================================
# NẠP API KEY TỪ config.json (TUỲ CHỌN)
# ============================================================
...
def _load_api_keys_from_config_json():
    global GEMINI_API_KEYS, QWEN_API_KEYS
    ...
    try:
        ...
        log(f"⚠️ Không đọc được config.json ({e}) — dùng key khai báo cứng trong code.",
            level="warning")
        return
...
# ============================================================
# LỚP TIỆN ÍCH: điều khiển ADB (với Stealth/Jitter)
# ============================================================

def run_adb_command(command: str, timeout: int = 20) -> str:
    """Chạy một lệnh adb shell, trả về stdout. Không raise, chỉ log lỗi."""
    try:
        result = subprocess.run(
            f"adb shell {command}", shell=True, capture_output=True, text=True, timeout=timeout
        )
        ...
        log(f"Lỗi khi chạy adb command '{command}': {e}", level="error")
        return ""
    ...
    with _device_action_lock:
        try:
            ...
            if long_press or (STEALTH_MODE and random.random() < STEALTH_LONG_PRESS_PROB):
                ...
                # Long press via swipe with same start/end but duration in ms
    ...
    with _device_action_lock:
        try:
            if STEALTH_MODE and human_like and STEALTH_HUMAN_TYPING:
                for ch in text:
                    # input text giải thích: space phải thay bằng %s, escape backslash và đôi quote
                    safe = ch
                    if safe == " ":
                        safe = "%s"
                    # Một số ký tự có thể gây lỗi trong input text; ta vẫn thử gửi.
...
def get_screen_text() -> str:
    try:
        ...
        dump_result = subprocess.run(
            f"adb shell cat {UI_DUMP_PATH}", shell=True, capture_output=True,
...
def detect_emulator(force_reload=False) -> bool:
...
def simulate_vibrate(duration_ms: int = 200):
    ...
    try:
        ...
        # Nếu trả về rỗng hoặc lỗi, fallback
        if not out:
            # fallback: nhiều swipe ngắn (không thực sự rung nhưng tạo chút hoạt động)
            for _ in range(3):
...
def simulate_shake():
    ...
    try:
        ...
        # a few rapid small swipes to simulate motion
        for _ in range(4):
...
# ============================================================
# LỚP 1a/1b (giữ nguyên) - xử lý Accessibility + OpenCV (giải quyết QC)
# ============================================================
...
def try_close_ad(ui_content: str) -> bool:
    ...
    log("Accessibility Tree không thấy nút đóng nhưng nghi ngờ có quảng cáo -> dùng OpenCV (phương án cuối) để tìm bằng ảnh.")
    return _try_close_ad_via_opencv()


# ============================================================
# OpenCV fallbacks (giữ nguyên logic, thêm try/except kỹ)
# ============================================================
...
def read_gray_reduced(path: str, scale: int = 1):
    flag = _REDUCED_GRAYSCALE_FLAGS.get(scale, cv2.IMREAD_GRAYSCALE)
        ...
        for fname in sorted(os.listdir(TEMPLATE_DIR)):
            ...
            try:
                ...
                if AD_DECODE_SCALE > 1:
                    h, w = tmpl.shape[:2]
                    tmpl = cv2.resize(
                        tmpl, (max(1, w // AD_DECODE_SCALE), max(1, h // AD_DECODE_SCALE)),
...
def _try_close_ad_via_opencv() -> bool:
    try:
        ...
        screen_small = read_gray_reduced(SCREENSHOT_TMP, AD_DECODE_SCALE)
        ...
        log(f"OpenCV fallback gặp lỗi, bỏ qua (không dừng agent): {e}", level="warning")
    return False
...
# ============================================================
# LỚP 2: GỌI API AI ĐỂ RA QUYẾT ĐỊNH / XÁC MINH (giữ nguyên)
# ============================================================
...
# ============================================================
# LỚP 2b: AI WORKER CHẠY NỀN (giữ nguyên)
# ============================================================
...
# ============================================================
# CƠ CHẾ AN TOÀN: tap có xác minh + khôi phục (sửa để dùng stealth typing)
# ============================================================

def safe_tap_with_recovery(x: int, y: int, expected_context: str, max_retries: int = 2) -> bool:
    # Nếu detect emulator và bật EMULATOR_BEHAVIOR_ENABLED, làm 1 hành vi mô phỏng nhỏ trước khi tap
    try:
        if EMULATOR_BEHAVIOR_ENABLED and detect_emulator():
            if EMULATOR_SHAKE_ENABLED:
                simulate_shake()
            # làm long press ngắn để "thử" trước khi tap thực
...
def main():
    ...
    # detect emulator once at startup
    ...
    while True:
        try:
                        ...
                        if decision.get("action") == "reply":
                            if not rate_limiter.allow():
                                ...
                                if x is None or y is None:
                                    ...
                                    # Nếu stealth mode & emulator -> có thể thực hiện hành vi đặc biệt
                                    if STEALTH_MODE and is_emulator and EMULATOR_BEHAVIOR_ENABLED:
                                        if EMULATOR_SHAKE_ENABLED:
                                            simulate_shake()
                                        # small delay before typing to mimic human
                                        time.sleep(random.uniform(0.2, 0.6))

                                    success = safe_tap_with_recovery(x, y, EXPECTED_CONTEXT)
                                    if success:
                                        # use human typing when stealth enabled
            ...
            diff_ratio = 1.0 if last_submitted_content is None else compute_text_diff_ratio(
